...
import MsgStats, { DailyCount, HourlyCount, HourlyCountByDate, TotalCountStats, WordFreqList, WordFrequency } from "./MsgStats";
import Util from "./Util";

class MsgStatsMain extends MsgStats {
  private msgCount: HourlyCountByDate;
  ...
  public getWordFrequency(filter: Function = ((word: string) => true)): WordFrequency {
    ...
    for (const entry of this.entryList) {
      ...
      wordList = MsgStatsMain.tokenizeText(entry.getText());
  ...
  public getReportTitle(): string {
    const appName = this.msgData.getAppName();
    const msgTarget = this.msgData.getMsgTarget();
    const userList = this.msgData.getUserList();
    let title: string;
